...
const Oracion247 = () => {
  ...
  const { value: guides, setValue: setGuides } = useDbStorage<PrayerGuide[]>('oracion247_guides', []);
  const { value: progress, setValue: setProgress } = useDbStorage<PrayerProgress[]>('oracion247_progress', []);
  const { value: history, setValue: setHistory } = useDbStorage<PrayerHistory[]>('oracion247_history', []);
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Oración 24/7</h1>
            <p className="text-muted-foreground mt-1">Guías de oración continua 24/7</p>
          </div>
